...
class GenerateWakandaLog extends Command
{
    ...
    public function handle()
    {
        ...
        $headers = ['MSISDN', 'Message', 'Transaction_Type', 'Millipede_Error', 'Error_Response', 'HTTP_Method', 'SKU', 'Status_Code', 'Activity_Date'];

        $mainWriter = WriterEntityFactory::createCSVWriter();

        $mainWriter->openToFile(public_path("$date/wakanda-logs-report-$date.csv"));
        ...
        collect($range)
            ->each(function ($range) use ($date, $mainWriter, $headers, &$overallCount) {
                ...
                WakandaLog::where('query_date', $date)
                    ...
                    ->chunk(5000, function ($records) use ($mainWriter, $subWriter, &$total, &$overallCount, $from) {
